...
export DIR=${DIR:-/services}

register_check() {
  declare check="$1"
  local name=$(jq -r .name <<< $check)
  local checkDef=$CONFIG_DIR/check-$name.json
  w() { echo "$@" >> $checkDef; }
  ...
  w '{
    "check": {
      "id": "'$name'",
      "script": "'$(jq -r .script <<< $check)'",
      "interval": "15s",
      "timeout": "5s",
      "status": "passing"
    }
  }'
  echo "check   registered: $name"
}

register_check   '{"name":"disk",  "script":"/usr/lib/nagios/plugins/check_disk -w 15% -c 10%"}'

discover_services() {
  find $DIR -name "*.yml" | xargs -n1 grep "^\s*- service" | grep -o '{.*}'
}

register_service() {
  declare service="$1"
  local name=$(jq -r .name <<< $service)
  local serviceDef=$CONFIG_DIR/service-$name.json
  w() { echo "$@" >> $serviceDef; }

  if [[ "$(jq -r '.checks[]' <<< $service)" == "" ]]; then
    echo "Fail to register service $name: check not found"
    exit 1
  fi
  ...
  w '{
    "service": {
      ...
      "checks": ['
        ...
        comma=,
        done < <(jq -r '.checks[]' <<< $service)
  w ']
    }
  }'
  jq . $serviceDef > /dev/null
